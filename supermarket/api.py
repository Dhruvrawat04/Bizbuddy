...
# Import mart1 integration
from mart1_integration import get_mart1_integration
...
def load_data():
    """
    Load the data from mart1 API (preferred) or uploaded file in the session.
    Returns a sample of the data and column information.
    If neither is available, returns sample data.
    """
    try:
        # Check if user explicitly uploaded a CSV file
        use_uploaded_csv = session.get('use_uploaded_csv', False)
        
        # If user uploaded CSV, use it instead of mart1
        if use_uploaded_csv and 'uploaded_file' in session:
            logging.info("User uploaded CSV - using uploaded file instead of mart1")
            ...
            return jsonify({
                ...
                'row_count': len(df),
                'data_source': 'csv',
                'note': 'Using your uploaded CSV file.'
            })
        
        # Otherwise, check if we should use mart1 data
        use_mart1 = config.get('data_sources', {}).get('prefer_mart1', True)
        mart1_enabled = config.get('mart1_api', {}).get('enabled', True)
        
        if use_mart1 and mart1_enabled and mart1_integration.is_available():
            logging.info("Loading data from mart1 API")
            
            # Fetch sales data from mart1
            days = config.get('analytics', {}).get('default_days', 30)
            df = mart1_integration.get_sales_data(
                limit=config.get('analytics', {}).get('max_records', 10000),
                days=days
            )
            
            if df is not None and not df.empty:
                # Save to temporary file for consistency with existing code
                import tempfile
                ...
                logging.info(f"Loaded {len(df)} records from mart1 API")
                ...
                return jsonify({
                    ...
                    'row_count': len(df),
                    'data_source': 'mart1',
                    'note': 'Using real-time data from mart1 billing system.'
                })
        ...
        if 'uploaded_file' in session:
            ...
            return jsonify({
                ...
                'row_count': len(df),
                'data_source': 'csv',
                'note': 'Using uploaded CSV file.'
            })
        
        # If no file is uploaded and mart1 is not available, return sample data
        logging.info("No file in session and mart1 not available - providing sample data")
...
def forecast_sales():
    ...
    try:
        ...
        try:
            ...
            # Add weekly seasonality component if it exists in forecast
            if 'weekly' in forecast.columns:
                components_data['weekly'] = {
                    'x': ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
                    'y': forecast['weekly'].tolist()[:7] if len(forecast['weekly']) >= 7 else [],
                    'name': 'Weekly Seasonality'
                }
            
            # Add yearly seasonality component if it exists in forecast
            if 'yearly' in forecast.columns:
                yearly_values = forecast['yearly'].tolist()
                if len(yearly_values) >= 12:
                    components_data['yearly'] = {
                        'x': list(range(1, 13)),
                        'y': yearly_values[:12],
            ...
            components_data = {
                'trend': {
                    'x': forecast['ds'].dt.strftime('%Y-%m-%d').tolist(),
                    'y': forecast['trend'].tolist() if 'trend' in forecast.columns else [],
                    'name': 'Trend'
                }
            }
...
# ============== MART1 INTEGRATION ENDPOINTS ==============

@api_bp.route('/mart1/status', methods=['GET'])
def check_mart1_status():
    """Check if mart1 API is available and return connection status"""
    try:
        is_available = mart1_integration.is_available()
        
        if is_available:
            # Try to get summary data
            summary = mart1_integration.get_dashboard_summary()
            
            return jsonify({
                'success': True,
                'available': True,
                'base_url': mart1_integration.base_url,
                'data_source': 'mart1',
            ...
            return jsonify({
                ...
                'available': False,
                'base_url': mart1_integration.base_url,
                'message': 'Mart1 API is not available. Using fallback data sources.'
            })
            
    except Exception as e:
        logging.error(f"Error checking mart1 status: {e}")
        return jsonify({
            'success': False,
            'available': False,
            'error': str(e)
...
@api_bp.route('/mart1/refresh-data', methods=['POST'])
def refresh_mart1_data():
    """Force refresh data from mart1 API"""
    try:
        if not mart1_integration.is_available():
            return jsonify({
                'success': False,
                'error': 'Mart1 API is not available'
            }), 503
        ...
        if df is not None and not df.empty:
            ...
            return jsonify({
                'success': True,
                'message': 'Data refreshed from mart1',
                'record_count': len(df)
            })
        else:
            return jsonify({
                'success': False,
                'error': 'No data received from mart1'
            }), 500
            
    except Exception as e:
        logging.error(f"Error refreshing mart1 data: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@api_bp.route('/switch-data-source', methods=['POST'])
def switch_data_source():
    """Switch between mart1 and CSV data sources"""
    try:
        data = request.json
        source = data.get('source', 'mart1')  # 'mart1' or 'csv'
        
        if source == 'mart1':
            # Switch to mart1 data
            if not mart1_integration.is_available():
                return jsonify({
                    'success': False,
                    'error': 'Mart1 API is not available'
            ...
            # Clear CSV flag and set mart1 as source
            session.pop('use_uploaded_csv', None)
            session['data_source'] = 'mart1'
            
            return jsonify({
                'success': True,
                'message': 'Switched to Mart1 data source',
                'data_source': 'mart1'
            })
            ...
            # Set CSV as the active source
            session['use_uploaded_csv'] = True
            session['data_source'] = 'csv'
            
            return jsonify({
                'success': True,
                'message': 'Switched to uploaded CSV data source',
                'data_source': 'csv'
            })
        else:
            return jsonify({
                'success': False,
                'error': f'Invalid data source: {source}. Use "mart1" or "csv"'
            }), 400
            
    except Exception as e:
        logging.error(f"Error switching data source: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
