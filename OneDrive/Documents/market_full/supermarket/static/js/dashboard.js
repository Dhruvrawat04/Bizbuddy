...
function updateDataSourceIndicator(data) {
    const alertElement = document.getElementById('dataSourceAlert');
    const textElement = document.getElementById('dataSourceText');
    
    if (!alertElement || !textElement) return;
    
    if (data.data_source === 'mart1') {
        alertElement.className = 'alert alert-success alert-dismissible fade show mb-3';
        textElement.innerHTML = '<strong>Mart1 Billing System</strong> (Real-time data) - ' + (data.note || '');
    } else if (data.data_source === 'csv') {
        alertElement.className = 'alert alert-info alert-dismissible fade show mb-3';
        textElement.innerHTML = '<strong>Uploaded CSV File</strong> - ' + (data.note || '');
    } else {
        alertElement.className = 'alert alert-warning alert-dismissible fade show mb-3';
        textElement.innerHTML = '<strong>Sample Data</strong> - ' + (data.note || 'Using demonstration data');
    }
}
