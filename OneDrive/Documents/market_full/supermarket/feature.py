...
def predict_sales_with_prophet(sales_data, periods=30, yearly_seasonality=True, weekly_seasonality=True, daily_seasonality=False):
    """
    Predict future sales using Exponential Smoothing (Holt-Winters method).
    ...
    seasonal_periods = 7 if weekly_seasonality else None
    if yearly_seasonality and len(df) > 365:
        seasonal_periods = 365
    
    trend = 'add' if len(df) > 10 else None
    seasonal = 'add' if seasonal_periods and len(df) > 2 * seasonal_periods else None
    ...
    try:
        ...
        all_forecasts = np.concatenate([historical_forecast, forecast_values])
        all_dates = pd.concat([df['ds'], pd.Series(future_dates)])
        actual_values = np.concatenate([df['y'].values, [np.nan] * periods])
        
        std_error = np.std(df['y'] - historical_forecast)
        lower_bound = all_forecasts - 1.96 * std_error
        upper_bound = all_forecasts + 1.96 * std_error
        
        forecast = pd.DataFrame({
            'ds': all_dates.reset_index(drop=True),
            'y': actual_values,
            'yhat': all_forecasts,
        ...
        forecast = df.copy()
        forecast['yhat'] = df['y']
        forecast['yhat_lower'] = df['y'] * 0.9
        forecast['yhat_upper'] = df['y'] * 1.1
        forecast['trend'] = df['y']
