...
def favicon():
    return send_file('static/assets/logo.png', mimetype='image/png')
...
def upload():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Mark that user explicitly uploaded a CSV, so we should use it instead of mart1
            ...
            try:
                ...
                flash('File successfully uploaded! Now using your CSV data for analysis.', 'success')
...
@app.route('/dashboard')
def dashboard():
    # Auto-load mart1 data if available
    ...
    # Fallback to requiring uploaded file
    if 'uploaded_file' not in session:
        flash('Please upload a file first or ensure mart1 API is running', 'warning')
